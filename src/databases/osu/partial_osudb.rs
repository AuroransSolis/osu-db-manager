...
impl PartialLoad<OsuDbMask, OsuDbLoadSettings> for PartialOsuDb {
    fn read_single_thread(settings: OsuDbLoadSettings, bytes: Vec<u8>) -> ParseFileResult<Self> {
        ...
        let (beatmaps, unknown_int) = if let Some(m) = mask.beatmap_mask {
            ...
            (
                None,
                maybe_read_int(mask.unknown_int, &bytes[bytes.len() - 4..bytes.len()], i)?,
            )
        };
        ...
        let (beatmaps, unknown_int) = if let Some(m) = mask.beatmap_mask {
            if version >= 20160408 {
                ...
                let beatmaps = beatmaps
                    ...
                    .collect::<Vec<_>>();
                let unknown_int =
                    maybe_read_int(mask.unknown_int, &bytes, &mut start.lock().unwrap())?;
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            let last_modification_time =
                maybe_read_datetime(mask.last_modification_time, bytes, i)?;
            let approach_rate =
                ModernWithEntrySize::maybe_read_arcshpod(mask.approach_rate, bytes, i)?;
            ...
            let overall_difficulty =
                ModernWithEntrySize::maybe_read_arcshpod(mask.overall_difficulty, bytes, i)?;
            ...
            let unknown_short =
                ModernWithEntrySize::maybe_read_unknown_short(mask.unknown_short, bytes, i)?;
