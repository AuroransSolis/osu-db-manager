...
pub trait PartialLoad<M: Mask, LS: QueryStruct<M>>: Sized {
    fn read_from_bytes(settings: LS, jobs: usize, bytes: Vec<u8>) -> ParseFileResult<Self> {
        if jobs == 1 {
    ...
    fn read_single_thread(settings: LS, bytes: Vec<u8>) -> ParseFileResult<Self>;
    fn read_multi_thread(settings: LS, jobs: usize, bytes: Vec<u8>) -> ParseFileResult<Self>;
}